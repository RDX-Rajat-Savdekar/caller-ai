...
import type { CoverageReelProps } from "./types";

export const COVERAGE_REEL_FPS = 30;
export const COVERAGE_REEL_DURATION = 240;
export const COVERAGE_REEL_WIDTH = 960;
export const COVERAGE_REEL_HEIGHT = 540;
...
export function CoverageReel({ dialed, reached, unaccounted, names = [] }: CoverageReelProps) {
  const frame = useCurrentFrame();
  const shownDialed = countUp(frame, 6, dialed);
  const shownReached = countUp(frame, 22, reached);
  const shownUnaccounted = countUp(frame, 38, unaccounted);
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: colors.canvas, fontFamily: reelFont, color: colors.ink }}>
      <div style={{ padding: 32, height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        <div style={{ fontSize: 15, color: colors.mute }}>The people you cannot reach are the job</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          <Stat label="Dialed" value={shownDialed} color={colors.ink} />
          <Stat label="Reached" value={shownReached} color={colors.safe} />
          <Stat label="Unreached" value={shownUnaccounted} color={colors.unaccounted} />
        </div>
        <div
          style={{
            opacity: listOpacity,
        ...
        >
          <div style={{ fontSize: 13, color: colors.mute, marginBottom: 12 }}>Voicemail is unreached</div>
